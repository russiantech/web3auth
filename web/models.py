...
class User(db.Model, UserMixin):
    ...
    wallets = db.relationship('Wallets', backref='user', lazy=True)
    wallet_types = db.relationship('Wallet_types', backref='user', lazy=True)
...
class Wallets(db.Model):
    ...
    type_id = db.Column(db.Integer, db.ForeignKey('wallet_types.id'), nullable=False)  # Foreign Key to Wallet_types
    
    type = db.relationship('Wallet_types', backref='wallets', lazy=True)  # Establish relationship
    ...
    def serialize(self):
        return {
            ...
            "title": self.type.title,  # Assuming the relationship exists
            "image": self.type.image,  # Assuming the relationship exists
